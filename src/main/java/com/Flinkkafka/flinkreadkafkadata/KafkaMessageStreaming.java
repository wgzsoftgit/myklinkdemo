...
import org.apache.flink.api.common.functions.FlatMapFunction;
   ...
   /**
* @Description    KafkaMessageStreaming Flink入口类，
* 封装了对于Kafka消息的处理逻辑。本例每1秒统计一次结果并写入到本地件或者打印出来
* 
*/
    ...
    //    args[0] = "test-0921";  //传入的是kafka中的topic
        ...
        consumer.assignTimestampsAndWatermarks(new MessageWaterEmitter()); //&&根据Kafka消息确定Flink的水位
    
        DataStream<Tuple2<String, Long>> keyedStream = env
                .addSource(consumer)
                .flatMap(new MessageSplitter())   //&&根据“，”分割取
                .keyBy(0)
